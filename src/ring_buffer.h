...
typedef struct so_ring_buffer_t {
	...
	pthread_mutex_t mutex; // protects acces to the ring buffer
	pthread_cond_t not_empty; // consumer wait on this condition when buff is empty
	pthread_cond_t not_full;  // producer wait on this condition when buff is full
	bool stopped;
